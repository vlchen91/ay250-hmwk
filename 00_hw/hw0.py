...
class Bear_Population(object):
	...
	def procreate(self,bear, other_bear):
		...
		cub = Bear(rand_name.next(),rand_gend.next(),parents=[bear,other_bear])
	...
	def possible_mates(self,bear):
		...
		rule6 = filter(lambda x: True if (x.parents == [god,god] or bear.parents == [god,god]) else x.parents != bear.parents, rule5)
		return rule6
...
adam = Bear('Adam','m',parents=[god,god])
eve = Bear('Eve','f',parents=[god,god])
mary = Bear('Mary','f',parents=[god,god])
